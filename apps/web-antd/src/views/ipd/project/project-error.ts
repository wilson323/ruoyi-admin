 ...
 * 历史分散的 PROJECT_CODE_TEXTS / projectErrorText 已迁入共享表（含 project 域专属默认）。
 * 消费方应改 import 自 '../_shared/ipd-error-text' 的 ipdErrorText 并传 domain='project'。
 * 本文件保留仅供旧 import 兼容。
 *
...
export function projectErrorText(error: unknown, options: ProjectErrorOptions = {}): string {
  return _ipdErrorText(error, { domain: 'project', ...options });
}

export const projectErrorTextWith = (codeTexts: Record<number, string>) =>
  withCodeTextOverrides(codeTexts, { domain: 'project' });
