...
import { traceDetail } from '#/api/monitor/trace';
...
const [BasicDrawer, drawerApi] = useVbenDrawer({
  async onOpenChange(open) {
    ...
    try {
      const detail = await traceDetail(traceId);
      detailData.value = detail;
      currentRun.value = detail.run || null;
      nodeList.value = detail.nodes || [];
...
function buildTree(nodes: DisplayTraceNode[]): TreeNode[] {
  ...
  for (const item of treeNodes) {
    const parentKey = String(item.raw.parentNodeId || '');
