/**
 * 落地场景接口（R149 录入/展示界面；后端 ScenariosController 待交付）。
 *
 * 设计：
 ...
 * - 列表 GET /api/v1/scenarios/landed（按 projectId 可选筛选），返回 List<LandedScenarioView>；
 * - 批量导入 POST /api/v1/scenarios/landed/import，body 为 List<LandedScenarioCreateReq>，
...
export function listLandedScenarios(query?: LandedScenarioQuery): Promise<LandedScenario[]> {
  return ipdGet<LandedScenario[]>('/scenarios/landed', query as Record<string, unknown>);
}
